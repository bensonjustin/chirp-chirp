...
function countLetters(counter, sample_text) {
  // FIX ME
  sample_text = sample_text.replace(/ /g, "");
  sample_text = sample_text.replace(/,/g, "");
  sample_text = sample_text.replace(/[.\s]+/g, "");
  sample_text = sample_text.toLowerCase();
  console.log(sample_text);
  if (sample_text === "") {
    return;
  } else {
    let char = sample_text.substring(sample_text.length - 1);
    counter[char]++;
    sample_text = sample_text.substring(0, sample_text.length - 1);
